...
class DataSystem {
    ...
    getCardByName(name) {
        ...
        return this.cards.find(card => {
            ...
            return firstName.includes(nameLower) || 
    ...
    searchCards(query) {
        ...
        return this.cards.filter(card => {
            ...
            const organization = card.officialDetails?.organization?.toLowerCase() || '';
            const role = card.officialDetails?.role?.toLowerCase() || '';
            
            // Search in personality
            const personalityType = card.personalityTraits?.personalityType?.toLowerCase() || '';
        ...
        return this.cards.filter(card => {
            ...
            for (const [key, value] of Object.entries(criteria)) {
                ...
                switch (key) {
                    ...
                    case 'cardType':
                        if (card.officialDetails?.cardType?.toLowerCase() !== value.toLowerCase()) {
                            return false;
                        }
                        break;
                        
                    case 'organization':
                        if (card.officialDetails?.organization?.toLowerCase() !== value.toLowerCase()) {
                            return false;
                        }
                        break;
                    ...
                    default:
                        ...
                        if (current === null || current.toString().toLowerCase() !== value.toString().toLowerCase()) {
                            return false;
                        }
                }
            }
    ...
    getStatistics() {
        const stats = {
            ...
            cardsWithGallery: this.cards.filter(card => card.media?.gallery?.length > 0).length,
