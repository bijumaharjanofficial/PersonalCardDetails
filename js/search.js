...
class SearchPage {
  ...
  initModal() {
    ...
    if (this.modalGalleryPrev) {
      this.modalGalleryPrev.addEventListener("click", () =>
        this.prevModalGalleryImage(),
      );
    }

    if (this.modalGalleryNext) {
      this.modalGalleryNext.addEventListener("click", () =>
  ...
  renderModalContent(card) {
    ...
    const infoFields = [
      {
        label: "Username",
        value: identity.username ? `@${identity.username}` : "",
      },
  ...
  // In the SearchPage class, update these methods:
  ...
  setupModalGalleryEvents() {
    ...
    if (this.modalGalleryPrev) {
      this.modalGalleryPrev.removeEventListener("click", this.boundModalPrev);
      this.modalGalleryPrev.addEventListener("click", () =>
        this.prevModalGalleryImage(),
      );
    }
    ...
    if (this.modalGalleryNext) {
      this.modalGalleryNext.removeEventListener("click", this.boundModalNext);
      this.modalGalleryNext.addEventListener("click", () =>
        this.nextModalGalleryImage(),
      );
    }
  ...
  setupModalTouchEvents() {
    ...
    this.modalGalleryTrack.removeEventListener(
      "touchstart",
      this.boundModalTouchStart,
    );
    this.modalGalleryTrack.removeEventListener(
      "touchmove",
      this.boundModalTouchMove,
    );
    this.modalGalleryTrack.removeEventListener(
      "touchend",
      this.boundModalTouchEnd,
    );

    // Bind methods
    this.boundModalTouchStart = this.handleModalTouchStart.bind(this);
    this.boundModalTouchMove = this.handleModalTouchMove.bind(this);
    this.boundModalTouchEnd = this.handleModalTouchEnd.bind(this);

    // Add new listeners
    this.modalGalleryTrack.addEventListener(
      "touchstart",
      this.boundModalTouchStart,
      { passive: true },
    );
    ...
    this.modalGalleryTrack.addEventListener(
      "mousedown",
      this.handleModalMouseDown.bind(this),
    );
    this.modalGalleryTrack.addEventListener(
      "mousemove",
      this.handleModalMouseMove.bind(this),
    );
    this.modalGalleryTrack.addEventListener(
      "mouseup",
      this.handleModalMouseUp.bind(this),
    );
  }
  ...
  handleModalTouchEnd() {
    ...
    if (absDiff > this.modalSwipeThreshold) {
      if (diff > 0) {
        this.nextModalGalleryImage();
      } else {
        this.prevModalGalleryImage();
  ...
  renderModalGallery() {
    ...
    this.modalGalleryTrack.style.transform = `translate3d(-${this.currentModalGalleryIndex * 100}%, 0, 0)`;
  ...
  nextModalGalleryImage() {
    ...
    this.currentModalGalleryIndex =
      this.currentModalGalleryIndex === this.modalGalleryImages.length - 1
        ? 0
        : this.currentModalGalleryIndex + 1;
