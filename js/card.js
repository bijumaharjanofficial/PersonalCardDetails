...
class CardPage {
    ...
    initAudioControls() {
        if (this.cardPlayPauseBtn) {
            this.cardPlayPauseBtn.addEventListener("click", () => this.toggleCardMusic());
        }
    ...
    renderCoreIdentity(card) {
        ...
        document.getElementById("cardLastName").textContent = identity.lastName ? ` ${identity.lastName}` : '';
        ...
        // Update details
        this.setDetailValue("cardDob", identity.dateOfBirth ? utils.formatDate(identity.dateOfBirth) : "");
        this.setDetailValue("cardAge", identity.age || (identity.dateOfBirth ? utils.calculateAge(identity.dateOfBirth) : ""));
    ...
    renderOfficialDetails(card) {
        ...
        const fields = [
            ...
            { key: "organization", label: "Organization", value: details.organization },
            { key: "role", label: "Role", value: details.role },
            { key: "issueDate", label: "Issue Date", value: details.issueDate ? utils.formatDate(details.issueDate) : "" },
            { key: "expiryDate", label: "Expiry Date", value: details.expiryDate ? utils.formatDate(details.expiryDate) : "" },
        ];
    ...
    renderFavorites(card) {
        ...
        const favoriteFields = [
            { key: "musicGenre", label: "Music Genre", icon: "fas fa-music", value: favorites.musicGenre },
            { key: "artist", label: "Artist", icon: "fas fa-user-music", value: favorites.artist },
            { key: "songName", label: "Favorite Song", icon: "fas fa-play-circle", value: favorites.songName },
            { key: "song", label: "Song File", icon: "fas fa-file-audio", value: favorites.song ? "✓ Has song file" : "" },
            { key: "anime", label: "Anime", icon: "fas fa-tv", value: favorites.anime },
            { key: "animeCharacter", label: "Anime Character", icon: "fas fa-user-ninja", value: favorites.animeCharacter },
            { key: "movieSeries", label: "Movie/Series", icon: "fas fa-film", value: favorites.movieSeries },
            { key: "game", label: "Game", icon: "fas fa-gamepad", value: favorites.game },
            { key: "food", label: "Food", icon: "fas fa-utensils", value: favorites.food },
            { key: "drink", label: "Drink", icon: "fas fa-wine-glass-alt", value: favorites.drink },
            { key: "color", label: "Color", icon: "fas fa-palette", value: favorites.color },
            { key: "quote", label: "Quote", icon: "fas fa-quote-right", value: favorites.quote },
        ];
    ...
    renderLifestyle(card) {
        ...
        const lifestyleFields = [
            { key: "hobbies", label: "Hobbies", icon: "fas fa-paint-brush", value: lifestyle.hobbies ? lifestyle.hobbies.join(", ") : "" },
            { key: "skills", label: "Skills", icon: "fas fa-tools", value: lifestyle.skills ? lifestyle.skills.join(", ") : "" },
            { key: "dreamDestination", label: "Dream Destination", icon: "fas fa-globe-americas", value: lifestyle.dreamDestination },
            { key: "biggestFear", label: "Biggest Fear", icon: "fas fa-ghost", value: lifestyle.biggestFear },
            { key: "biggestMotivation", label: "Biggest Motivation", icon: "fas fa-rocket", value: lifestyle.biggestMotivation },
        ];
    ...
    renderStoryMode(card) {
        ...
        const storyFields = [
            ...
            { key: "makesHappy", label: "What Makes Them Happy", value: story.makesHappy },
            { key: "breaksThem", label: "What Breaks Them", value: story.breaksThem },
            { key: "protectAtAllCosts", label: "Protect At All Costs", value: story.protectAtAllCosts },
            { key: "secretSoftSpot", label: "Secret Soft Spot", value: story.secretSoftSpot },
        ];
    ...
    setupGalleryEvents() {
        ...
        if (this.galleryPrevBtn) {
            this.galleryPrevBtn.addEventListener("click", () => this.prevGalleryImage());
        }
        
        if (this.galleryNextBtn) {
            this.galleryNextBtn.addEventListener("click", () => this.nextGalleryImage());
        }
        
        // Add touch events
        if (this.galleryViewport) {
            this.galleryViewport.addEventListener("touchstart", this.handleTouchStart, { passive: true });
            this.galleryViewport.addEventListener("touchmove", this.handleTouchMove, { passive: false });
            this.galleryViewport.addEventListener("touchend", this.handleTouchEnd, { passive: true });
        }
        ...
        if (this.galleryViewport) {
            ...
            this.galleryViewport.addEventListener("mouseleave", this.handleMouseLeave);
        }
    }
    ...
    removeGalleryEventListeners() {
        ...
        if (this.galleryViewport) {
            this.galleryViewport.removeEventListener("touchstart", this.handleTouchStart);
            this.galleryViewport.removeEventListener("touchmove", this.handleTouchMove);
            this.galleryViewport.removeEventListener("touchend", this.handleTouchEnd);
            this.galleryViewport.removeEventListener("mousedown", this.handleMouseDown);
            this.galleryViewport.removeEventListener("mousemove", this.handleMouseMove);
            this.galleryViewport.removeEventListener("mouseup", this.handleMouseUp);
            this.galleryViewport.removeEventListener("mouseleave", this.handleMouseLeave);
        }
    }
    ...
    prevGalleryImage() {
        ...
        this.currentGalleryIndex = this.currentGalleryIndex === 0 ? this.galleryImages.length - 1 : this.currentGalleryIndex - 1;
        this.updateGalleryTransform();
        this.updateGalleryCounter();
    }
    ...
    nextGalleryImage() {
        ...
        this.currentGalleryIndex = this.currentGalleryIndex === this.galleryImages.length - 1 ? 0 : this.currentGalleryIndex + 1;
        this.updateGalleryTransform();
        this.updateGalleryCounter();
    }
    ...
    updateFooterInfo(card) {
        ...
        if (card.officialDetails?.issueDate) {
            document.getElementById("cardIssueDate").textContent = `Issued: ${utils.formatDate(card.officialDetails.issueDate)}`;
        }
    }
