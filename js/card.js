...
class CardPage {
    ...
    init() {
        ...
        this.dataSystem.addEventListener('dataloaded', () => this.loadCardFromURL());
    }
    ...
    renderCoreIdentity(card) {
        ...
        // Update details
        this.setDetailValue('cardDob', identity.dateOfBirth ? this.utils.formatDate(identity.dateOfBirth) : '');
        this.setDetailValue('cardAge', identity.age || (identity.dateOfBirth ? this.utils.calculateAge(identity.dateOfBirth) : ''));
    ...
    renderOfficialDetails(card) {
        ...
        const fields = [
            ...
            { key: 'issueDate', label: 'Issue Date', value: details.issueDate ? this.utils.formatDate(details.issueDate) : '' },
            { key: 'expiryDate', label: 'Expiry Date', value: details.expiryDate ? this.utils.formatDate(details.expiryDate) : '' }
        ];
    ...
    renderPersonalityTraits(card) {
        ...
        if (traits.weaknesses && traits.weaknesses.length > 0) {
            const weaknessesItem = this.utils.createElement('div', 'personality-item');
    ...
    renderFavorites(card) {
        ...
        const favoriteFields = [
            ...
            { key: 'song', label: 'Song', icon: 'fas fa-play-circle', value: favorites.song ? 'Has theme song' : '' },
        ...
        favoriteFields.forEach(field => {
            if (field.value) {
                ...
                if (field.key === 'color' && this.themeSystem.isValidColor(field.value)) {
    ...
    renderGallery() {
        ...
        this.galleryTrack.style.transform = `translateX(-${this.currentGalleryIndex * 100}%)`;
    ...
    nextGalleryImage() {
        ...
        this.currentGalleryIndex = this.currentGalleryIndex === this.galleryImages.length - 1 ? 
            0 : 
            this.currentGalleryIndex + 1;
    ...
    handleCardMusic(card) {
        ...
        // Update track info
        const songName = songUrl.split('/').pop().replace('.mp3', '').replace('-theme', ' Theme');
        this.cardTrackInfo.textContent = `Playing: ${songName}`;
    ...
    updateFooterInfo(card) {
        ...
        if (card.officialDetails?.issueDate) {
            document.getElementById('cardIssueDate').textContent = `Issued: ${this.utils.formatDate(card.officialDetails.issueDate)}`;
        }
    }
