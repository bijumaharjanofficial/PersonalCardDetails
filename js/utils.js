// Shared utility functions - UPDATED VERSION
class Utils {
  constructor() {
  ...
  init() {
    ...
    console.log("Utils initialized (Audio interaction removed)");
  }
  ...
  handleTouchMove(e) {
    ...
    const diff =
      this.currentGallery.swipeStartX - this.currentGallery.swipeEndX;
  ...
  setupGallerySwipe(galleryElement, nextCallback, prevCallback, images = []) {
    ...
    // Add new listeners
    galleryElement.addEventListener("touchstart", this.handleTouchStart, {
      passive: true,
    });
    galleryElement.addEventListener("touchmove", this.handleTouchMove, {
      passive: false,
    });
    galleryElement.addEventListener("touchend", this.handleTouchEnd, {
      passive: true,
    });
  ...
  // Simple audio status check (doesn't trigger audio)
  checkAudioStatus() {
